...
Script Version: 1.2 (added mass graves as per MadHatter's request)
*/
...
_heliModel 		= ["UH1Y_DZE","UH1H_DZE","MV22","Mi17_DZ","Mass_grave_DZ"] call BIS_fnc_selectRandom;		//The type of heli used
...
if(_heliModel == "Mass_grave_DZ") then {
    _lootTable 		= "MassGrave";
	_crashModel 	= "Mass_grave_DZ";
	_massGraveSite 	= true;
};
...
if (_spawnRoll <= _spawnChance) then 
{
	...
	{
		diag_log(format["CRASHSPAWNER: MassGrave spawned at %1",str(_position), _lootTable]);
	...
	else
	{
		...
		_crashwreck setVariable ["ObjectID","1",true];
